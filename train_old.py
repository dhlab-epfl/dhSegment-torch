...
def default_config():
    ...
    prediction_type = (
        utils.PredictionType.CLASSIFICATION
    )  # One of CLASSIFICATION, REGRESSION or MULTILABEL
    pretrained_model_name = "resnet50"
    model_params = utils.ModelParams(
        pretrained_model_name=pretrained_model_name
    ).to_dict()  # Model parameters
    ...
    if prediction_type == utils.PredictionType.CLASSIFICATION:
        ...
        assert classes_file is not None
        model_params["n_classes"] = utils.get_n_classes_from_file_multilabel(
            classes_file
        )
...
def run(train_data, eval_data, model_output_dir, gpu, training_params, _config):
    ...
    if not os.path.isdir(model_output_dir):
        ...
        assert _config.get("restore_model"), (
            ...
            'Set "restore_model=True" to continue training, or delete dir "rm -r {0}"'.format(
                model_output_dir
            )
        )
    # Save config
    ...
    estimator_config = tf.estimator.RunConfig().replace(
        session_config=session_config, save_summary_steps=10, keep_checkpoint_max=1
    )
    estimator = tf.estimator.Estimator(
        estimator_fn.model_fn,
    ...
    def get_dirs_or_files(input_data):
        if os.path.isdir(input_data):
            image_input, labels_input = (
                os.path.join(input_data, "images"),
                os.path.join(input_data, "labels"),
            )
            # Check if training dir exists
            assert os.path.isdir(image_input), "{} is not a directory".format(
                image_input
            )
            assert os.path.isdir(labels_input), "{} is not a directory".format(
                labels_input
            )
            ...
            raise TypeError(
                "input_data {} is neither a directory nor a csv file".format(input_data)
            )
    ...
    if eval_data is not None:
        exporter = tf.estimator.BestExporter(
            serving_input_receiver_fn=serving_input_fn, exports_to_keep=2
        )
    else:
    ...
    if not os.listdir(saved_model_dir):
        final_exporter = tf.estimator.FinalExporter(
            name="FinalExporter", serving_input_receiver_fn=serving_input_fn
        )
        final_exporter.export(
            estimator,
            saved_model_dir,
            checkpoint_path=None,
            eval_result=eval_result,
