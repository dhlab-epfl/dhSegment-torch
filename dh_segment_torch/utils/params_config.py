...
class PredictionType:
    ...
    @classmethod
    def parse(cls, prediction_type: str):
...
class ModelParams(BaseParams):
    ...
    def __init__(self, **kwargs):
        ...
        self.pretrained_model_name = kwargs.get(
        ...
        self.pretrained_model_file = kwargs.get(
            "pretrained_model_file", model_class.PRETRAINED_MODEL_FILE
        )
        self.intermediate_conv = kwargs.get(
            "intermediate_conv", model_class.INTERMEDIATE_CONV
        )
        self.upscale_params = kwargs.get("upscale_params", model_class.UPSCALE_PARAMS)
        self.selected_levels_upscaling = kwargs.get(
            "selected_levels_upscaling", model_class.SELECTED_LAYERS_UPSCALING
        )
        self.correct_resnet_version = kwargs.get(
            "correct_resnet_version", model_class.CORRECT_VERSION
        )
        self.check_params()

    def check_params(self):
        if (
            self.upscale_params is not None
            and self.selected_levels_upscaling is not None
        ):
            assert len(self.upscale_params) == len(self.selected_levels_upscaling), (
                ...
                "{} != {}".format(
                    len(self.upscale_params), len(self.selected_levels_upscaling)
                )
            )

        if not os.path.isfile(self.pretrained_model_file):
            warnings.warn(
                f"WARNING - Default pretrained weights file in {self.pretrained_model_file} "
...
class TrainingParams(BaseParams):
    ...
    def __init__(self, **kwargs):
        ...
        self.local_entropy_ratio = kwargs.get("local_entropy_ratio", 0.0)
        self.local_entropy_sigma = kwargs.get("local_entropy_sigma", 3)
        self.focal_loss_gamma = kwargs.get("focal_loss_gamma", 0.0)
    ...
    def check_params(self) -> None:
        ...
        if self.evaluate_every_epoch >= self.n_epochs:
            warnings.warn(
                f"Model will be trained for {self.n_epochs} but will not be evaluated until the end "
...
class DataParams(BaseParams):
    ...
    def __init__(self, **kwargs):
        ...
        self.input_resized_size = int(
            kwargs.get("input_resized_size", 72e4)
        )  # (600*1200)
        self.prediction_type = self._assign_prediction_type(
            kwargs.get("prediction_type", "CLASSIFICATION")
        )
        ...
        self.data_augmentation_horizontal_flip = kwargs.get(
            "data_augmentation_flip_lr", False
        )
        self.data_augmentation_vertical_flip = kwargs.get(
            "data_augmentation_flip_ud", False
        )
        self.data_augmentation_color = kwargs.get("data_augmentation_color", False)
        self.data_augmentation_max_rotation = kwargs.get(
            "data_augmentation_max_rotation", 10
        )
        self.data_augmentation_max_scaling = kwargs.get(
            "data_augmentation_max_scaling", 1.5
        )
    ...
    def _get_classes_from_file(self):
        ...
        if self.prediction_type == PredictionType.CLASSIFICATION:
            ...
            assert (
                content.shape[1] > 3
            ), "The number of columns should be greater in multilabel framework"
    ...
    def check_params(self):
        ...
        if not self.data_augmentation:
            logging.info(
                "Data augmentation is disabled. All augmentation parameters will be disabled."
            )
            ...
            assert (
                self.data_augmentation_max_scaling >= 1.0
            ), "Scaling factor should be greater or equal to 1.0"
            if self.data_augmentation_max_scaling < 1.0:
                self.data_augmentation_max_scaling = 1.0
                logging.error(
                    f"Scaling factor should be greater or equal to 1.0. "
            ...
            if self.data_augmentation_max_rotation > 180:
                ...
                logging.error(
                    f"Rotation angle should be lower or equal to 180. "
                    f"Changed its value to {self.data_augmentation_max_rotation}"
                )

            if (
                int(self.input_resized_size / self.data_augmentation_max_scaling)
                < self.minimum_input_size
            ):
                self.data_augmentation_max_scaling = (
                    self.input_resized_size / self.minimum_input_size
                )
                logging.error(
                    f"Scaling factor exceeded minimum image size. "
                    f"Changed it value to {self.data_augmentation_max_scaling}"
                )
            if (
                int(self.input_resized_size * self.data_augmentation_max_scaling)
                > self.maximum_input_size
            ):
                self.data_augmentation_max_scaling = (
                    self.maximum_input_size / self.input_resized_size
                )
                logging.error(
                    f"Scaling factor exceeded maximum image size. "
                    f"Changed it value to {self.data_augmentation_max_scaling}"
                )
