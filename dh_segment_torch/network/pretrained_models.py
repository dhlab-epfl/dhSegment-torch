...
def mean_substraction(input_tensor, means=_VGG_MEANS):
    return tf.subtract(
        input_tensor, np.array(means)[None, None, None, :], name="MeanSubstraction"
    )


def vgg_16_fn(
    input_tensor: tf.Tensor, scope="vgg_16", blocks=5, weight_decay=0.0005
...
def resnet_v1_50_fn(
    ...
    with slim.arg_scope(
        nets.resnet_v1.resnet_arg_scope(
            weight_decay=weight_decay, batch_norm_decay=0.999
        )
        ...
        if corrected_version:
            ...
            def corrected_resnet_v1_block(scope, base_depth, num_units, stride):
                """Helper function for creating a resnet_v1 bottleneck block.
                  ...
                  """
                return nets.resnet_utils.Block(
                    scope,
            ...
            blocks_list = [
                nets.resnet_v1.resnet_v1_block(
        ...
        net, endpoints = nets.resnet_v1.resnet_v1(
            input_tensor,
