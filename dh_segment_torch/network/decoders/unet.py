...
def get_channels_reduce(
    channels, max_channels: int = None, normalizer_fn: nn.Module = nn.Identity
) -> (nn.Module, int):
    """
    ...
    if max_channels and channels > max_channels:
        reduce = Conv2DNormalize(
            channels, max_channels, kernel_size=1, normalizer_fn=normalizer_fn
        )
    else:
        reduce = nn.Identity()
    ...
    def __init__(
        ...
        for enc_channels, dec_channels in zip(encoder_channels[1:], decoder_channels):
            ...
            ops["reduce_dim"], enc_channels = get_channels_reduce(
                enc_channels, max_channels, normalizer_fn
            )
            ...
            conv = Conv2DNormalize(
                prev_channels + enc_channels, dec_channels, normalizer_fn=normalizer_fn
            )
