...
class UnetTest(DhSegmentTestCase):
    def test_encoders_from_params(self):
        ...
        xs = [torch.zeros(4, c, 512//(i+1), 512//(i+1)).normal_() for i,c in enumerate(channels)]
        ...
        assert decoder.level_ops[0]['decoder_conv'][0][0].in_channels == channels[-1] + channels[-2]
        assert decoder.level_ops[0]['decoder_conv'][0][0].out_channels == channels[-1]
        assert decoder.level_ops[1]['decoder_conv'][0][0].in_channels == channels[-1] + channels[-3]
        assert decoder.level_ops[1]['decoder_conv'][0][0].out_channels == channels[-2]
        assert decoder.level_ops[2]['decoder_conv'][0][0].in_channels == channels[-2] + channels[-4]
        assert decoder.level_ops[2]['decoder_conv'][0][0].out_channels == channels[-3]
        assert decoder.level_ops[3]['decoder_conv'][0][0].in_channels == channels[-3] + channels[-5]
        ...
        decoder = Decoder.from_params(params)
        assert decoder.level_ops[0]['decoder_conv'][0][0].in_channels == min(max_channels, channels[-1]) + channels[-2]
        assert isinstance(decoder.level_ops[0]['up_concat'].deconv, torch.nn.ConvTranspose2d)
