...
from typing import Type, TypeVar, cast, Callable, Dict, Union, Any, Mapping, Tuple, Set, Iterable, List
...
class FromParams:
    ...
    def from_params(cls: Type[T],
        ...
        if not is_base_registrable(cls):
            raise RegistrableError("Only direct subclasses of Registrable are allowed in arguments of function")
...
def create_kwargs(constructor: Callable[..., T], cls: Type[T], params: Params, **extras):
    kwargs: Dict[str, any] = {}
    ...
    for param_name, param in parameters.items():
        ...
        popped_params = params.pop(param_name, param.default) if optional else params.pop(param_name)
        kwargs[param_name] = construct_param(param_name, param, popped_params, **extras)
    print(params.as_dict())
    params.assert_empty(cls.__name__)
    return kwargs


def construct_param(param_name: str, param: inspect.Parameter, params: Params, **extras):
    """
    ...
    if param_name in extras:
        if param_name not in params:
            ...
            logger.warning(f"Parameter {param_name} was found in extras, which is not required,"
    ...
    if has_from_params(param_type):
        if params is param.default:
            ...
            params = normalize_params(params)
            print("here", param_name, params.as_dict())
    ...
    elif origin in {collections.abc.Mapping, Mapping, Dict, dict} and len(args) == 2 and can_construct(args[-1]):
        if not isinstance(params, collections.abc.Mapping):
            raise TypeError(f"Expected {param_name} to be a mapping.")
        value_class = args[-1]
        value_class_as_param = inspect.Parameter("dummy", kind=inspect.Parameter.VAR_KEYWORD, annotation=value_class)

        new_dict = {}
        for key_param, value_params in params.items():
            new_dict[key_param] = construct_param(f"{param_name}.{key_param}",
        ...
        for i, (value_class, value_params) in enumerate(zip(args, params)):
            value_class_as_param = inspect.Parameter("dummy", kind=inspect.Parameter.VAR_KEYWORD,
                                                     annotation=value_class)
            new_tuple.append(construct_param(f"{param_name}.{i}", value_class_as_param, value_params, **extras))
        return tuple(new_tuple)
    elif origin in {Set, set} and len(args) == 1 and can_construct(args[0]):
        if not isinstance(params, collections.abc.Set) and not isinstance(params, collections.abc.Sequence):
        ...
        for i, value_params in enumerate(params):
            new_set.add(construct_param(f"{param_name}.{i}", value_class_as_param, value_params, **extras))
        return new_set
    elif origin in {collections.abc.Iterable, Iterable, List, list} and len(args) == 1 and can_construct(args[0]):
        if not isinstance(params, collections.abc.Sequence):
            raise TypeError(f"Expected {param_name} to be a set.")
        value_class = args[0]
        value_class_as_param = inspect.Parameter("dummy", kind=inspect.Parameter.VAR_KEYWORD, annotation=value_class)
        ...
        for i, value_params in enumerate(params):
            new_list.append(construct_param(f"{param_name}.{i}", value_class_as_param, value_params, **extras))
        return new_list
    elif origin == Union:
        ...
        for value_class in args:
            value_class_as_param = inspect.Parameter("dummy", kind=inspect.Parameter.VAR_KEYWORD,
                                                     annotation=value_class)
            try:
                return construct_param(param_name, value_class_as_param, params, **extras)
            except (AttributeError, ValueError, TypeError, ConfigurationError, RegistrableError):
                params = deepcopy(backup_params)
        raise ConfigurationError(f"Failed to construct {param_name} wit type {param_type}")
    else:
        logger.warning(f"The params {params} were not matched, returning them as is.")
...
def infer_params(constructor: Callable[..., T], cls: Type[T]):
    ...
    for super_class_candidate in inspect.getmro(cls)[1:]:
        if issubclass(super_class_candidate, FromParams):
            ...
            break
    if not super_class:
        raise RuntimeError("Found a kwargs parameter with no superclass to account for them.")
...
def infer_type(param: inspect.Parameter) -> type:
    if param.annotation == param.empty:
        # If the param did not have a type annotation, try to see if it had a default value
        if param.default != param.empty:
            return type(param.default)
        else:
            raise RegistrableError("Got a registrable that had params either not type hinted either w/o defaults")
    else:
        origin, args = get_origin_args(param.annotation)
...
def get_extras(cls: Type[T], extras: Dict[str, Any]) -> Dict[str, Any]:
    class_from_params = getattr(cls, "from_params", None)
    if class_from_params is None:
        raise RegistrableError(f"Class {cls.__name__} requesting extras should have from params mixin.")
