...
def compute_with_shapes(input_tensor, shapes, reduce=torch.mean, margin=0):
    ...
    for idx in range(shapes.shape[0]):
        shape = shapes[idx]
        res += reduce(cut_with_padding(input_tensor[idx], shape))  # .item()
    res = reduce(res)
    return res


def patch_loss_with_padding(loss_class):
    class LossPatchedWithPadding(loss_class):
        ...
        def forward(self, input, target):
            ...
            return compute_with_shapes(loss, shapes, margin=self.margin)
...
def patch_metric_with_padding(metric_class, margin=0):
    class MetricWithPadding(metric_class):
        def update(self, output):
            ...
            for idx in range(shapes.shape[0]):
                shape = shapes[idx]
                y_pred_tmp = cut_with_padding(y_pred[idx], shape).unsqueeze(0).contiguous()
                y_tmp = cut_with_padding(y[idx], shape).unsqueeze(0).contiguous()
