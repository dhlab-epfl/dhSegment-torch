...
class InputDataset(Dataset, ABC):
    ...
    def __init__(self, dataframe: pd.DataFrame, transform: tsfm.Compose = None):
    ...
    def __getitem__(self, idx: int):
        ...
        sample = {
            "image": self.dataframe.images.iloc[idx],
...
class InputCSVDataset(InputDataset):
    ...
    def __init__(self, csv_filename: str, transform: tsfm.Compose = None):
        super().__init__(
            dataframe=pd.read_csv(
                csv_filename, header=None, names=["images", "labels"]
            ),
            transform=transform,
        )
...
class InputFolderDataset(InputDataset):
    ...
    def __init__(self, folder: str, transform: tsfm.Compose = None):
        ...
        super().__init__(
            dataframe=pd.DataFrame(data=input_data, columns=["images", "labels"]),
            transform=transform,
        )
...
class InputListCSVDataset(InputDataset):
    ...
    def __init__(self, csv_list=List[str], transform: tsfm.Compose = None):

        list_dataframes = list()
        for csv_file in csv_list:
            assert os.path.isfile(csv_file), f"{csv_file} does not exist"
            list_dataframes.append(
                pd.read_csv(csv_file, header=None, names=["images", "labels"])
            )
