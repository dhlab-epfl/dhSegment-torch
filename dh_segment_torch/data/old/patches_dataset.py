    ...
    def __iter__(self):
        ...
        for samples in batch_items(
            ...
            samples = [
                ...
                for patch in sample_to_patche_samples(sample, self.patch_size, offset_augment=self.offsets_augment)
            ]
...
def extract_patches(image, patch_shape=(300, 300), overlap=(None, None)):
    ...
    patches = patches.reshape(-1, patch_h, patch_w, image.shape[2]) if len(image.shape) == 3 else patches.reshape(-1, patch_h, patch_w)
    return patches
