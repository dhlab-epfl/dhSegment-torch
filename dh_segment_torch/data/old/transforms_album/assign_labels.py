...
class AssignLabelMultilabel(DualTransform):
    ...
    def __init__(self, colors_array: np.ndarray, onehot_label_array: np.ndarray):
        ...
        super().__init__(always_apply=True, p=1)
        self.colors_array = colors_array
        self.colors_array = (colors_array*[1000*1000, 1000, 1]).sum(axis=-1)[None, :]#.astype(np.uint8)
        self.onehot_label_array = onehot_label_array#.astype(np.uint8)
...
class AssignOneHot(DualTransform):
    ...
    def __init__(self, onehot_label_array: np.ndarray):
        super().__init__(always_apply=True, p=1)
        self.onehot_label_array = onehot_label_array#.astype(np.uint8)
