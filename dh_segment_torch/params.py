...
class ModelParams(BaseParams):
    def __init__(self, **kwargs):
        self.encoder_network = kwargs.get(
            "encoder_network", "dh_segment_torch.network.encoders.Resnet50Encoder"
        )
        self.encoder_params = kwargs.get("encoder_params", dict())
        self.decoder_network = kwargs.get(
            "decoder_network", "dh_segment_torch.network.decoders.UnetDecoder"
        )
        self.decoder_params = kwargs.get(
            "decoder_params",
            {"decoder_channels": [512, 256, 128, 64, 32], "max_channels": 512},
        )
...
class TrainingParams(BaseParams):
    ...
    def __init__(self, **kwargs):
        ...
        self.device = (
            kwargs.get("device", "cpu") if torch.cuda.is_available() else "cpu"
        )
        self.non_blocking = kwargs.get("non_blocking", True)
        self.pin_memory = kwargs.get("pin_memory", True)
        self.model_out_dir = kwargs.get("model_out_dir", "./model")
        self.tensorboard_log_dir = kwargs.get(
            "tensorboard_log_dir", os.path.join(self.model_out_dir, "logs")
        )
        ...
        self.train_checkpoint_interval = kwargs.get(
